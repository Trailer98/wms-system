-- V1__init_schema.sql
-- Full WMS schema baseline, migrated verbatim from the legacy schema.sql (which used to be
-- auto-run by Spring Boot on every startup via spring.sql.init.mode=always).
-- Runs ONCE on an empty database. Existing databases are baselined at V1 (this is skipped).
-- Contains only DDL + idempotent information_schema-guarded ALTERs + operation_type/adjust_action
-- data-normalisation UPDATEs (no-ops on an empty DB). No DROP DATABASE/DROP TABLE/TRUNCATE/business DELETE.
...
create table if not exists warehouse_locations (
    ...
    allow_mixed_sku tinyint not null default 1,
    pick_priority integer not null default 0,
    remark varchar(255),
    enabled tinyint not null,
    created_at timestamp not null,
...
SET @stmt := (SELECT IF((SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'sys_operation_log' AND column_name = 'success') = 0,
    'ALTER TABLE sys_operation_log ADD COLUMN success tinyint NOT NULL DEFAULT 1', 'SELECT 1'));
PREPARE stmt FROM @stmt;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
